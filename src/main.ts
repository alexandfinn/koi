...
function createParticle(
  ...
  return {
    ...
    opacity:
      (0.1 + Math.random() * 0.3 * depthOpacityFactor) * opacityMultiplier, // Much less opacity for deeper particles
    depth: depth,
  };
}
...
function createParticleLayers(
  ...
  for (let i = 0; i < numLayers; i++) {
    ...
    const particlesInThisLayer =
      depth === 0 ? Math.floor(particlesPerLayer / 2) : particlesPerLayer;
...
function drawCreature(ctx: CanvasRenderingContext2D, creature: CreatureNode[]) {
  ...
  const cp1x =
    startX +
    Math.cos(creature[dorsalFinStartIndex].angle - angleOffset) *
      dorsalFinHeight;
  const cp1y =
    startY +
    Math.sin(creature[dorsalFinStartIndex].angle - angleOffset) *
      dorsalFinHeight;
  const cp2x =
    endX +
    Math.cos(creature[dorsalFinEndIndex].angle - angleOffset) * dorsalFinHeight;
  const cp2y =
    endY +
    Math.sin(creature[dorsalFinEndIndex].angle - angleOffset) * dorsalFinHeight;
  ...
  const firstFinAngle =
    firstFinNode.angle +
    leftFin.angle -
    (firstFinNode.angle - firstPredNode.angle) * 3.2;
  ...
  const secondFinAngle =
    secondFinNode.angle +
    backLeftFin.angle -
    (secondFinNode.angle - secondPredNode.angle) * 3.2;
  ...
  ctx.moveTo(
    creature[0].x +
      Math.cos(creature[0].angle - Math.PI / 2) * creature[0].radius * 0.9,
    creature[0].y +
      Math.sin(creature[0].angle - Math.PI / 2) * creature[0].radius * 0.9
  );

  // Head's left intermediate - more rounded
  ctx.lineTo(
    creature[0].x +
      Math.cos(creature[0].angle - Math.PI / 4) * creature[0].radius * 0.95,
    creature[0].y +
      Math.sin(creature[0].angle - Math.PI / 4) * creature[0].radius * 0.95
  );
  ...
  ctx.lineTo(
    creature[creature.length - 1].x +
      Math.cos(creature[creature.length - 1].angle + Math.PI / 2) *
        creature[creature.length - 1].radius *
        0.9,
    creature[creature.length - 1].y +
      Math.sin(creature[creature.length - 1].angle + Math.PI / 2) *
        creature[creature.length - 1].radius *
        0.9
  );
  ...
  ctx.lineTo(
    creature[creature.length - 1].x +
      Math.cos(creature[creature.length - 1].angle + (3 * Math.PI) / 4) *
        creature[creature.length - 1].radius *
        0.8,
    creature[creature.length - 1].y +
      Math.sin(creature[creature.length - 1].angle + (3 * Math.PI) / 4) *
        creature[creature.length - 1].radius *
        0.8
  );
  ...
  ctx.lineTo(
    creature[creature.length - 1].x +
      Math.cos(creature[creature.length - 1].angle + Math.PI) *
        creature[creature.length - 1].radius *
        0.7,
    creature[creature.length - 1].y +
      Math.sin(creature[creature.length - 1].angle + Math.PI) *
        creature[creature.length - 1].radius *
        0.7
  );
  ...
  ctx.moveTo(
    creature[startNodeIndex].x +
      Math.cos(creature[startNodeIndex].angle) *
        creature[startNodeIndex].radius *
        0.5,
    creature[startNodeIndex].y +
      Math.sin(creature[startNodeIndex].angle) *
        creature[startNodeIndex].radius *
        0.5
  );
  ...
  const tailStartX =
    creature[tailFinStartIndex].x - Math.cos(tailTipAngle) * tailExtension;
  const tailStartY =
    creature[tailFinStartIndex].y - Math.sin(tailTipAngle) * tailExtension;
  ...
  const tailAngleOffset =
    (-Math.PI / 2) * curvatureDirection * smoothCurvature(normalizedCurvature);

  // Calculate control points for the tail fin Bezier curve
  // Changed shape to be broader as it extends back
  const tailCp1x =
    tailStartX +
    Math.cos(tailTipAngle - Math.PI + tailAngleOffset) * tailFinHeight;
  const tailCp1y =
    tailStartY +
    Math.sin(tailTipAngle - Math.PI + tailAngleOffset) * tailFinHeight;

  // Create a broader tail by having the second control point extend further and wider
  const tailCp2x =
    tailEndX +
    Math.cos(tailTipAngle - Math.PI + tailAngleOffset) * (tailFinHeight * 2);
  const tailCp2y =
    tailEndY +
    Math.sin(tailTipAngle - Math.PI + tailAngleOffset) * (tailFinHeight * 2);
  ...
  const sideViewStartX =
    tailStartX +
    Math.cos(sideViewAngle) *
      sideViewWidth *
      smoothCurvature(curvatureDirection * normalizedCurvature);
  const sideViewStartY =
    tailStartY +
    Math.sin(sideViewAngle) *
      sideViewWidth *
      smoothCurvature(curvatureDirection * normalizedCurvature);
  ...
  const baseSideAngle =
    tailTipAngle -
    Math.PI +
    (Math.PI / 6) * smoothCurvature(curvatureDirection * normalizedCurvature);

  // Use a more gradual curve for the side view
  const sideViewCp1x =
    sideViewStartX + Math.cos(baseSideAngle) * sideViewHeight * 0.4;
  const sideViewCp1y =
    sideViewStartY + Math.sin(baseSideAngle) * sideViewHeight * 0.4;

  const sideViewCp2x =
    sideViewEndX + Math.cos(baseSideAngle) * sideViewHeight * 0.4;
  const sideViewCp2y =
    sideViewEndY + Math.sin(baseSideAngle) * sideViewHeight * 0.4;
  ...
  const leftEyeX =
    creature[0].x +
    Math.cos(creature[0].angle) * leftEye.offsetX -
    Math.sin(creature[0].angle) * leftEye.offsetY;
  const leftEyeY =
    creature[0].y +
    Math.sin(creature[0].angle) * leftEye.offsetX +
    Math.cos(creature[0].angle) * leftEye.offsetY;
  ...
  const rightEyeX =
    creature[0].x +
    Math.cos(creature[0].angle) * rightEye.offsetX -
    Math.sin(creature[0].angle) * rightEye.offsetY;
  const rightEyeY =
    creature[0].y +
    Math.sin(creature[0].angle) * rightEye.offsetX +
    Math.cos(creature[0].angle) * rightEye.offsetY;
  // Draw elliptical eye for foreshortening effect
  ctx.ellipse(
    rightEyeX,
    rightEyeY,
  ...
  const leftWhiskerX =
    creature[0].x +
    Math.cos(creature[0].angle) * -5 -
    Math.sin(creature[0].angle) * -12;
  const leftWhiskerY =
    creature[0].y +
    Math.sin(creature[0].angle) * -5 +
    Math.cos(creature[0].angle) * -12;
...
function moveTowardsTarget(node: CreatureNode, target: Target, speed: number) {
  ...
  if (distance > 0.1) {
    ...
    const distanceScale =
      distance < 120 ? 0 : Math.pow(Math.min(1, (distance - 120) / 100), 2);
    ...
    const oscillation =
      (primaryOscillation + secondaryOscillation) * distanceScale;
...
function updateFollowerNode(follower: CreatureNode, leader: CreatureNode) {
...
function drawFood(ctx: CanvasRenderingContext2D, food: Food) {
  ...
  const currentSize =
    food.initialSize - (food.initialSize - food.targetSize) * progress;
  ...
  for (const point of points) {
    ...
    if (point.disturbance) {
      ...
      if (elapsed < point.disturbance.duration) {
        ...
        const strength =
          point.disturbance.strength * easeOutCubic * strengthMultiplier;
    ...
    point.x =
      point.baseX +
      Math.cos(time * point.speed + point.phase) * point.amplitude +
      xOffset;
    point.y =
      point.baseY +
      Math.sin(time * point.speed * 1.5 + point.phase) * point.amplitude +
      yOffset;
  }
}
...
function applyFishDisturbance(
  ...
  const speed = Math.sqrt(
    head.speedX * head.speedX + head.speedY * head.speedY
  );
  ...
  for (const point of points) {
    ...
    if (distance < disturbanceRadius) {
      ...
      const currentStrength = point.disturbance
        ? point.disturbance.strength
        : 0;
      ...
      if (strength > currentStrength) {
        ...
        const duration =
          baseDuration * (1 + distance / disturbanceRadius) +
          Math.random() * 500;
...
function animate() {
  ...
  for (const food of foods) {
    if (!food.eaten && food.age >= 0) {
      // Only react to food with positive age
